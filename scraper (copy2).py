...
from instaloader.structures import PostComment
import random 
import requests
...
class Post(Post):
    def get_comments(self) -> Iterator[PostComment]:
        ...
        if self.comments <= 4:
            print('overloaded Post Class: comments < 5')# Avoid doing additional requests if there are no comments
            return
        try:
